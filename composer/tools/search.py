...
class RAGDBContext(Protocol):
    ...
    @property
    def cvlr_rag_db(self) -> PostgreSQLRAGDatabase | None:
        ...
...
class CVLRManualSearchSchema(WithToolCallId):
    """
    Search the CVLR manual database for information relevant to a question about CVLR (Certora Verification Language for Rust).

    This tool uses semantic similarity search to find the most relevant documentation
    sections from the CVLR/Solana manual that can help answer questions about CVLR syntax,
    Solana prover usage, and best practices for verifying Rust/Solana programs.

    The result is a list of quotes from the manual, identified with the name of the relevant section.
    """
    question: str = Field(description="A single, self-contained question about CVLR or the Solana prover. Avoid open-ended 'how do I...?' questions in favor of 'What is the syntax for ...?' style questions.")
    similarity_cutoff: float = Field(default=0.5, description="Minimum cosine similarity threshold for results (default: 0.5)")
    max_results: int = Field(default=10, description="Maximum number of search results to return (default: 10)")
    manual_section: List[str] = \
        Field(default=[], description="A list of manual sections to search. "
              "If specified, at least one section heading must match at least one of the values provided here")
...
@tool(args_schema=CVLRManualSearchSchema)
def cvlr_manual_search(
    question: str,
    tool_call_id: Annotated[str, InjectedToolCallId],
    similarity_cutoff: float = 0.5,
    max_results: int = 10,
    ...
    """Search the CVLR/Solana manual database for relevant documentation."""
    runtime = get_runtime(RAGDBContext)
    writer = get_stream_writer()

    if runtime.context.cvlr_rag_db is None:
        return "CVLR RAG database is not configured. Please ensure cvlr_rag_db is initialized."

    try:
        to_ret: List[SearchResultSchema] = []
        for t in runtime.context.cvlr_rag_db.find_refs(query=question, similarity_cutoff=similarity_cutoff, top_k=max_results, manual_section=manual_section):
            upd : ManualSearchResult = {
                "type": "manual_search",
                "tool_id": tool_call_id,
                "ref": t
        ...
        return f"Failed to search CVLR manual: {str(e)}"
